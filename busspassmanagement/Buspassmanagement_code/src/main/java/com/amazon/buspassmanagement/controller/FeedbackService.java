...
public class FeedbackService {
	...
	public void createSuspendRequestBP() {
		Feedback feedback = new Feedback();
		feedback.getSuspendBP();
		
		feedback.userId = BusPassSession.user.id;
		feedback.raisedBy = BusPassSession.user.email;
		
		int result = feedbackDAO.insert(feedback);
		String message = (result > 0) ? "BusPass suspension request Created Successfully" : "Creating request Failed. Try Again.."; 
